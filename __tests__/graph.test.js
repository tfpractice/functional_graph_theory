...
import { addEdges, addEntry, addNeighbor, addNodes, adj, contains, copy,
  graph, isAdjacent, mergeEdges, neighbors, nodes,
...
export const oGraph = graph(...oFilter(myNodes));

describe('graph', () => {
  describe('graph', () => {
    it('returns a new edgelist with the new nodes appended', () => {
      expect(graph(...myNodes) instanceof Map).toBeTrue();
    });
  });
  describe('accessors', () => {
    describe('nodes', () => {
