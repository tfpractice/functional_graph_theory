fdescribe('Graph', function() {
	...
	describe('accessors', () => {
		describe('nodes', () => {
			it(
				'returns a set of the nodes from that graph',
				() => {
					expect(nodes(myGraph) instanceof Set)
						.toBeTrue();
				});
		});
		describe('edges', () => {
			it('returns a Map of nodes and neighbor', () => {
				// console.log(edges(myGraph).get(n0));
				expect(edges(myGraph) instanceof Map)
					.toBeTrue();
			});
		});
		describe('neighbors(edge)(node)', () => {
			it(
				'returns a map entry of that nodes neighbors ',
				() => {
					expect(neighbors(myGraph)(n0) instanceof Array)
						.toBeTrue();
				});
		});
	});
	describe('operators', () => {
		describe('contains(graph)(node)', () => {
			it(
				'checks for the presence of a node in the graph',
				() => {
		...
		describe('addNodes', () => {
			it(
				'adds an edges entry for each additional node',
				() => {
		...
		describe('removeNodes', () => {
			it(
				'removes a node from all its neihghbors nad the graph',
				() => {
					addNodes(myGraph)(n7, n9);
		...
		describe('addEdge(n0)(n1,weight)', () => {
			it('updates each nodes edge entry', () => {
				expect(neighbors(myGraph)(n0)).toContain(
					n1);
				expect(edges(myGraph).get(n0).get(
					n1)).toBe(2);
			});
		});
		describe('importEdge(n0)(n1,weight)', () => {
			it('updates each nodes edge entry', () => {
				let firstOdd = [...oddGraph.edges
					.entries(),
				][0];
				let OddNode = firstOdd[0];
				let OddNabe = firstOdd[1];
				importEdge(myGraph)(firstOdd);
				expect(contains(myGraph)(
					OddNode)).toBeTrue();
			});
		});
		describe('removeEdge', () => {
			it('removes an entry from the edgs map', () => {
				...
				expect(neighbors(myGraph)(n0)).not
					.toContain(n1);
				expect(edges(myGraph).get(n0).get(
					n1)).toBeUndefined();
			});
