describe('traversal functions', () => {
	...
	describe('unvisitedNeighbors', () => {
		it('returns nodes not in path', () => {
			...
			let currNabes = trav.unvisitedNeighbors(
				myGraph)(localPath)(n0);
	...
	describe('unvisitedMap', () => {
		it('returns a map of nodes not in path', () => {
			let localPath = new Map().set(n0, {});
			let currNabes = (trav.unvisitedMap(myGraph)
				(localPath)(n0));
	...
	describe('trav.dfs', () => {
		it('returns a map of nodes and neighbors', () => {
			expect((trav.dfs(myGraph)(n0) instanceof Map))
				.toBeTrue();
		});
	});
	describe('bfs', () => {
		it('returns a map of nodes and neighbors', () => {
			expect((trav.bfs(myGraph)(n0) instanceof Map))
				.toBeTrue();
		});
	});
	describe('dijkstra', () => {
		it(
			'retuns the shortest path from a node to its neighbors',
			() => {
				expect((trav.dijkstra(myGraph)(n0) instanceof Map))
					.toBeTrue();
			});
	});
	describe('components', () => {
		it('retuns a map of nodes and paths', () => {
			expect((trav.components(myGraph) instanceof Map))
				.toBeTrue();
		});
	});
